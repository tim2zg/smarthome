...
class eigverbdata extends StatefulWidget {
  @override
...
class sadfwegs42 extends State<eigverbdata> {
  @override
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          title: const Text('Eigen Verbrauch'),
        ),
        body: TabBarView(
          children: [
            FutureBuilder(
                future: eigverbdatagetig("1d", "1m"),
            ...
            FutureBuilder(
                future: eigverbdatagetig("7d", "10m"),
                builder: (BuildContext context,AsyncSnapshot snapshot){
                  if(snapshot.connectionState == ConnectionState.done) {
                    return charts.TimeSeriesChart(
                      snapshot.data,
            ...
            FutureBuilder(
                future: eigverbdatagetig("1mo", "30m"),
                builder: (BuildContext context,AsyncSnapshot snapshot){
                  if(snapshot.connectionState == ConnectionState.done) {
                    return charts.TimeSeriesChart(
                      snapshot.data,
            ...
            FutureBuilder(
                future: eigverbdatagetig("12mo", "6h"),
                builder: (BuildContext context,AsyncSnapshot snapshot){
                  if(snapshot.connectionState == ConnectionState.done) {
                    return charts.TimeSeriesChart(
                      snapshot.data,
